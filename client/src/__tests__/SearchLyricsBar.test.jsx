...
describe('SearchLyricsBar', () => {
  ...
  it('should render search results after pressing the button', async () => {
    const searchAPIMock = LyricService.search.mockResolvedValueOnce(items);
    const { getByTestId } = render(<SearchLyricsBar />);
  ...
  it(`doesn't render search results when data cannot be retrieved`, async () => {
    ...
    const searchAPIMock = LyricService.search.mockRejectedValueOnce(
      new SearchLyricsError(`Cannot find lyrics. ${errorMessage}`)
    );
    const { getByTestId } = render(<SearchLyricsBar />);
    const nativeButton = getByTestId(TestIds.SEARCH_LYRICS_BUTTON_ID);
    fireEvent.click(nativeButton);
    await waitFor(() => expect(searchAPIMock).toHaveBeenCalledTimes(1));
    expect(() =>
      getByTestId(TestIds.SEARCH_LYRICS_RESULTS_ARTICLE_ID)
    ).toThrowError();
  });
  ...
  it(`renders error's article when data cannot be retrieved`, async () => {
    ...
    await waitFor(() => expect(searchAPIMock).toHaveBeenCalledTimes(1));
    expect(
      getByTestId(TestIds.SEARCH_LYRICS_ERROR_ARTICLE_ID)
    ).toBeInTheDocument();
  });
  ...
  it(`renders error code when data cannot be retrieved`, async () => {
    ...
    await waitFor(() => expect(searchAPIMock).toHaveBeenCalledTimes(1));
    expect(
      getByTestId(TestIds.SEARCH_LYRICS_ERROR_CODE_ID)
    ).toBeInTheDocument();
    expect(getByTestId(TestIds.SEARCH_LYRICS_ERROR_CODE_ID)).toHaveTextContent(
      ErrorCodes.SEARCH_LYRICS_ERROR
    );
  });
  ...
  it(`renders error message when data cannot be retrieved`, async () => {
    ...
    await waitFor(() => expect(searchAPIMock).toHaveBeenCalledTimes(1));
    expect(
      getByTestId(TestIds.SEARCH_LYRICS_ERROR_MESSAGE_ID)
    ).toHaveTextContent(`Cannot find lyrics. ${errorMessage}`);
  });
});
